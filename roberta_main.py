...
from roberta_data_loader import OriginalWebQSPDataLoader, NameExpandedWebQSPDataLoader, WebREDForPretrainingDataLoader, OriginalWebQSPWithPromptDataLoader, MixedWebQSPWebREDDataLoader, SplitedPathWebQSPDataLoader
from roberta_model import BertForKBQA
...
def parse_args():
    ...
    parser.add_argument("--mix_bertrl_data", action="store_true", help="Mix BERTRL data when training KBQA")
...
def main(args):
            ...
            for batch_idx in trange_obj:
                ...
                if (batch_idx + 1) % args.evaluation_steps == 0 or batch_idx == num_batches - 1:
                    ...
                    if args.metric in ("hits", "f1"):
                        threshold, p, r, f1, hits, loss = evaluate_kbqa(dev_loader, model, args.batch_size, args.max_seq_length, thresholds=args.threshold, log_file=os.path.join(args.model_save_path, f"evaluate_dev_step{global_step}.json"), multi_paths=(args.task == "kbqa_multi_paths" and args.use_aggregate))
    ...
    model.cuda()
    if args.metric in ("hits", "f1"):
        threshold, p, r, f1, hits, loss = evaluate_kbqa(test_loader, model, args.batch_size, args.max_seq_length, thresholds=best_dev_threshold, log_file=os.path.join(args.model_save_path, "evaluate_test.json"), multi_paths=(args.task == "kbqa_multi_paths" and args.use_aggregate))
        logging.info(f"Results: f1 = {f1:.6f} (threshold = {threshold:.6f}), hits@1 = {hits:.6f}")
    elif args.metric == "accuracy":
