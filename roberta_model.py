...
class BertForKBQA(torch.nn.Module):
    def __init__(self,
        ...
        self.bert = RobertaModel.from_pretrained(model_name_or_path)
...
class BertForPretraining(torch.nn.Module):
    def __init__(self,
        ...
        assert pretraining_task in ("matching", "masked_token_prediction", "joint")        
        self.bert = RobertaModel.from_pretrained(model_name_or_path)
        
        if pretraining_task in ("matching", "joint"):
            self.linear = torch.nn.Linear(768, 1)
            assert pooling_strategy in ("max_pooling", "cls_token", "mean_pooling")
            self.pooling_strategy = pooling_strategy
    ...
    def forward(self, input_ids, token_type_ids, attention_mask):
        ...
        if self.pretraining_task in ("matching", "joint"):
            if self.pooling_strategy == "max_pooling":
                input_mask_expanded = attention_mask.unsqueeze(-1).expand(token_embeddings.size()).float()
                token_embeddings[input_mask_expanded == 0] = -1e9  # Set padding tokens to large negative value
                pooled_output = torch.max(token_embeddings, 1)[0]
