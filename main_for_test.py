...
def parse_args():
    ...
    parser.add_argument("--output_filename", type=str, default="evaluate_test.json")
...
def main(args):
    ...
    if args.metric in ("hits", "f1"):
        threshold, p, r, f1, hits, loss = evaluate_kbqa(args, test_loader, model, args.batch_size, args.max_seq_length, thresholds=0.2, log_file=os.path.join(args.model_path, args.output_filename), multi_paths=(args.task == "kbqa_multi_paths" and args.use_aggregate))
        logging.info(f"Results: f1 = {f1:.6f} (threshold = {threshold:.6f}), hits@1 = {hits:.6f}")
    elif args.metric == "accuracy":
        acc = evaluate_accuracy(test_loader, model, args.batch_size, args.max_seq_length, log_file=os.path.join(args.model_path, args.output_filename))
