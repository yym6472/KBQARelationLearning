...
class NameExpandedWebQSPDataLoader(object):
    ...
    def __init__(self, file_path: str, split: str, bert_model_name_or_path: str, shuffle: bool = True, entity_start_token: str = None, entity_end_token: str = None, head_entity_start_token: str = None, head_entity_end_token: str = None, tail_entity_start_token: str = None, tail_entity_end_token: str = None, unknown_entity_token: str = "[unknown entity]", separate_token: str = "[SEP]", annotate_self_token: str = None, use_prompt: bool = False, mix_webred_data: str = None, mix_webred_matching_data: str = None, mix_bertrl_data: str = None, use_logging=True, bertrl_max_samples: int = 1000000):
        ...
        samples_for_matching = []
        num_total_samples, num_pos_samples, num_neg_samples = 0, 0, 0
        for q_id, sample in samples.items():
            ...
            for candidate in sample["candidiates"]:  # a typo when processing data file
                ...
                num_total_samples += 1
                if candidate["is_answer"] == 0:
                    num_neg_samples += 1
                else:
                    num_pos_samples += 1
        logging.info(f"Number of KBQA samples for matching: total={num_total_samples}; pos={num_pos_samples} ({100 * num_pos_samples / num_total_samples:.2f}%); neg={num_neg_samples} ({100 * num_neg_samples / num_total_samples:.2f}%)")
        if mix_webred_data and split == "train":
            num_total_samples, num_pos_samples, num_neg_samples = 0, 0, 0
            assert shuffle, "Mixed WebRED data during training but not shuffle"
            webred_file = mix_webred_data
            ...
            for sample in webred_samples:
                for item in sample["true"] + sample["false"]:
                    ...
                    if item["label"] == 0:
                        ...
                        num_pos_samples += 1
            logging.info(f"Number of RE samples for matching: total={num_total_samples}; pos={num_pos_samples} ({100 * num_pos_samples / num_total_samples:.2f}%); neg={num_neg_samples} ({100 * num_neg_samples / num_total_samples:.2f}%)")
        if mix_webred_matching_data and split == "train":
            num_total_samples, num_pos_samples, num_neg_samples = 0, 0, 0
            assert shuffle, "Mixed WebRED matching data during training but not shuffle"
            webred_file = mix_webred_matching_data
            webred_samples = json.load(open(webred_file, "r"))
            for sample in webred_samples:
                samples_for_matching.append({
                ...
                if sample["label"] == 0:
                    ...
                    num_pos_samples += 1
            logging.info(f"Number of RM samples for matching: total={num_total_samples}; pos={num_pos_samples} ({100 * num_pos_samples / num_total_samples:.2f}%); neg={num_neg_samples} ({100 * num_neg_samples / num_total_samples:.2f}%)")
        if mix_bertrl_data and split == "train":
            num_total_samples, num_pos_samples, num_neg_samples = 0, 0, 0
            assert shuffle, "Mixed BERTRL data during training but not shuffle"
            bertrl_file = mix_bertrl_data
            with open(bertrl_file, "r") as f:
                for idx, line in enumerate(f):
                    if idx >= bertrl_max_samples:
                        break
                    ...
                    if int(label) == 0:
                        ...
                        num_pos_samples += 1
            logging.info(f"Number of RR samples for matching: total={num_total_samples}; pos={num_pos_samples} ({100 * num_pos_samples / num_total_samples:.2f}%); neg={num_neg_samples} ({100 * num_neg_samples / num_total_samples:.2f}%)")
        if shuffle:
            random.shuffle(samples_for_matching)
        self.data = samples_for_matching
        self.num_data = len(self.data)
        ...
        self.tokenizer = BertTokenizer.from_pretrained(bert_model_name_or_path, additional_special_tokens=self.special_tokens)
...
class PWNameExpandedWebQSPDataLoader(object):
    
    def __init__(self, file_path: str, split: str, bert_model_name_or_path: str, shuffle: bool = True, entity_start_token: str = None, entity_end_token: str = None, head_entity_start_token: str = None, head_entity_end_token: str = None, tail_entity_start_token: str = None, tail_entity_end_token: str = None, unknown_entity_token: str = "[unknown entity]", separate_token: str = "[SEP]", annotate_self_token: str = None, use_prompt: bool = False, mix_webred_data: str = None, mix_webred_matching_data: str = None, mix_bertrl_data: str = None, use_logging=True):
        samples = json.load(open(file_path, "r"))
        if use_logging:
            logging.info(f"Reading {split} split from {file_path}, number of samples: {len(samples)}")
        else:
        ...
        for q_id, sample in samples.items():
            ...
            for candidate in sample["candidiates"]:  # a typo when processing data file
                ...
                samples_for_matching.append({
                    "question": question_with_prompt if use_prompt else question,
                    "paths_pos": self._convert_and_combine_paths(candidate["converted_paths_pos"], candidate["is_self_pos"], annotate_self_token, entity_start_token, entity_end_token, head_entity_start_token, head_entity_end_token, tail_entity_start_token, tail_entity_end_token, separate_token),
                    "paths_neg": self._convert_and_combine_paths(candidate["converted_paths"], candidate["is_self"], annotate_self_token, entity_start_token, entity_end_token, head_entity_start_token, head_entity_end_token, tail_entity_start_token, tail_entity_end_token, separate_token),
                    "id": q_id,
                    "label": candidate["is_answer"],
                    "candi": candidate["candidate_id"]
                })
    ...
    def get_batch(self, iteration, batch_size, max_length):
        ...
        bert_inputs = self.tokenizer.batch_encode_plus([[sample["question"], sample["paths_pos"]] for sample in batch_data], padding=True, truncation=True, max_length=max_length, return_tensors="pt")
        input_ids_pos = bert_inputs["input_ids"]
        token_type_ids_pos = bert_inputs["token_type_ids"]
        attention_mask_pos = bert_inputs["attention_mask"]
        bert_inputs = self.tokenizer.batch_encode_plus([[sample["question"], sample["paths_neg"]] for sample in batch_data], padding=True, truncation=True, max_length=max_length, return_tensors="pt")
        input_ids_neg = bert_inputs["input_ids"]
        token_type_ids_neg = bert_inputs["token_type_ids"]
        attention_mask_neg = bert_inputs["attention_mask"]
        
        return input_ids_pos, token_type_ids_pos, attention_mask_pos, input_ids_neg, token_type_ids_neg, attention_mask_neg, q_ids, candidates
...
class PWWebREDMatchingDataLoader(object):
    """
    两句话判断是否具有同一个关系的预训练任务，路径：./data/webred_matching
    """
    def __init__(self, file_path: str, split: str, bert_model_name_or_path: str, shuffle: bool = True):
    ...
    def get_batch(self, iteration, batch_size, max_length):
        ...
        bert_inputs = self.tokenizer.batch_encode_plus([[sample["text"], sample["pos"]] for sample in batch_data], padding=True, truncation=True, max_length=max_length, return_tensors="pt")
        input_ids_pos = bert_inputs["input_ids"]
        token_type_ids_pos = bert_inputs["token_type_ids"]
        attention_mask_pos = bert_inputs["attention_mask"]
        bert_inputs = self.tokenizer.batch_encode_plus([[sample["text"], sample["neg"]] for sample in batch_data], padding=True, truncation=True, max_length=max_length, return_tensors="pt")
        input_ids_neg = bert_inputs["input_ids"]
        token_type_ids_neg = bert_inputs["token_type_ids"]
        attention_mask_neg = bert_inputs["attention_mask"]
        
        return input_ids_pos, token_type_ids_pos, attention_mask_pos, input_ids_neg, token_type_ids_neg, attention_mask_neg
...
class WebREDMaskedTokenPredictionDataLoader(object):
    ...
    def __init__(self, file_path: str, split: str, bert_model_name_or_path: str, shuffle: bool = True):
        ...
        # 统计高频词
        self.tokenizer = BertTokenizerFast.from_pretrained(bert_model_name_or_path)
...
class BERTRLMatchingDataLoader(object):
    ...
    def __init__(self, file_path: str, split: str, bert_model_name_or_path: str, shuffle: bool = True):
        ...
        self.data = samples
        self.num_data = len(self.data)
        self.tokenizer = BertTokenizer.from_pretrained(bert_model_name_or_path, additional_special_tokens=["[unknown entity]", "[path separator]", "[start of entity]", "[end of entity]", "[start of head entity]", "[end of head entity]", "[start of tail entity]", "[end of tail entity]", "[self]"])
...
class JointMatchingDataLoader(object):
    """
    用于三个任务（RE、RM、RR）联合预训练。
    """
    def __init__(self, split: str, bert_model_name_or_path: str, shuffle: bool = True, mix_webred_data: bool = True, mix_bertrl_data: bool = True, mix_webred_matching_data: bool = True, bertrl_max_samples: int = 1000000):
        selection = {
            "webred": mix_webred_data,
            "webred_matching": mix_webred_matching_data,
            "bertrl": mix_bertrl_data
        }
        paths = {
            "webred": {
                "root": "./data/webred/",
                "train": "webred_21_pretraining_2hop.json",
                "dev": "webred_5_pretraining_2hop.json",
                "test": "webred_5_pretraining_2hop.json"
            },
            "webred_matching": {
                "root": "./data/webred_matching/",
                "train": "webred_21_for_matching.json",
                "dev": "webred_5_for_matching.json",
                "test": "webred_5_for_matching.json"
            },
            "bertrl": {
                "root": "./data/freebase_pretraining_bertrl/webqsp/",
                "train": "train.tsv",
                "dev": "dev.tsv",
                "test": "test.tsv"
            }
        ...
        samples_for_matching = []
        for dataset_name, selected in selection.items():
            if not selected:
                continue
            file_to_load = os.path.join(paths[dataset_name]["root"], paths[dataset_name][split])
            if dataset_name == "webred":
                num_total_samples, num_pos_samples, num_neg_samples = 0, 0, 0
                assert shuffle or split != "train", "Mixed WebRED data during training but not shuffle"
                webred_file = file_to_load
                webred_samples = json.load(open(webred_file, "r"))
                for sample in webred_samples:
                    for item in sample["true"] + sample["false"]:
                        samples_for_matching.append({
                        ...
                        if item["label"] == 0:
                            ...
                            num_pos_samples += 1
                logging.info(f"Number of RE samples for matching: total={num_total_samples}; pos={num_pos_samples} ({100 * num_pos_samples / num_total_samples:.2f}%); neg={num_neg_samples} ({100 * num_neg_samples / num_total_samples:.2f}%)")
            if dataset_name == "webred_matching":
                num_total_samples, num_pos_samples, num_neg_samples = 0, 0, 0
                assert shuffle or split != "train", "Mixed WebRED matching data during training but not shuffle"
                webred_file = file_to_load
                webred_samples = json.load(open(webred_file, "r"))
                for sample in webred_samples:
                    samples_for_matching.append({
                    ...
                    if sample["label"] == 0:
                        ...
                        num_pos_samples += 1
                logging.info(f"Number of RM samples for matching: total={num_total_samples}; pos={num_pos_samples} ({100 * num_pos_samples / num_total_samples:.2f}%); neg={num_neg_samples} ({100 * num_neg_samples / num_total_samples:.2f}%)")
            if dataset_name == "bertrl":
                num_total_samples, num_pos_samples, num_neg_samples = 0, 0, 0
                assert shuffle or split != "train", "Mixed BERTRL data during training but not shuffle"
                bertrl_file = file_to_load
                with open(bertrl_file, "r") as f:
                    for idx, line in enumerate(f):
                        if idx >= bertrl_max_samples:
                            break
    ...
    def get_batch(self, iteration, batch_size, max_length):
        ...
        for sample in batch_data:
            ...
            texts_to_encode.append([sample["question"], sample["paths"]])
